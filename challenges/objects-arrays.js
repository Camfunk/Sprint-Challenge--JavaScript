...
const unisWithUni = graduates.filter(items => items.university.includes("Uni"));
...
*/
const displayNames = [];
zooAnimals.forEach(animalia => {
  displayNames.push(
    `Name ${animalia.animal_name} Scientific ${animalia.sicentific_name}.`
  );
});
...
let total = zooAnimals.reduce((accumulated, zooAnimals) => {
  return (populationTotal = accumulated + zooAnimals.population);
}, 0);
...
Stretch: If you haven't already, convert your array method callbacks into arrow functions. DONE
